...
class Game {
  ...
  def checkForBaseDamage(): Unit = {
    for(player <- players.values){
      if(player.location.distance2d(new PhysicsVector(level.base.x + 0.5,level.base.y + 0.5)) <= .3){
        baseHealth = baseHealth - 1
        player.location.x = level.startingLocation.x + 0.5
        player.location.y = level.startingLocation.y + 0.5
      }
    }
  }
  ...
  def checkForPlayerHits(): Unit = {
    for(player <- players.values){
      for(pewpew <- projectiles){
        if(player.location.distance2d(pewpew.location) <= .3){
          pewpew.destroy()
          player.location.x = level.startingLocation.x + 0.5
          player.location.y = level.startingLocation.y + 0.5
        }
